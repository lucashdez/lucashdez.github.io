...
export class Lissajous {
  ...
  #drawGrid() {
      ...
      this.#context.fillText('0', this.#canvas.width / 2, this.#canvas.height / 2);
      let xAugments = 0.1;
      for(let separator = this.#canvas.width / 2; separator < this.#canvas.width; separator += 60) {
        if (separator === this.#canvas.width / 2) continue;
        this.#context.fillText(xAugments.toFixed(1), separator, this.#canvas.width / 2 - 160);
        xAugments += 0.1
      }
      xAugments = -0.1;
      for(let separator = this.#canvas.width / 2; separator > 0; separator -= 60) {
        if (separator === this.#canvas.width / 2) continue;
        this.#context.fillText(xAugments.toFixed(1), separator, this.#canvas.width / 2 - 160);
        xAugments -= 0.1;
      }
      let yAugments = -0.1;
      for(let separator = this.#canvas.height / 2; separator < this.#canvas.height; separator += 60) {
        if (separator === this.#canvas.height / 2) continue;
        this.#context.fillText(yAugments.toFixed(1), this.#canvas.height / 2 + 100, separator);
        yAugments += 0.1
      }
      yAugments = 0.1;
      for(let separator = this.#canvas.height / 2; separator > 0; separator -= 60) {
        if (separator === this.#canvas.height / 2) continue;
        this.#context.fillText(yAugments.toFixed(1), this.#canvas.height / 2 + 100, separator);
        yAugments += 0.1;
      }
   ...
   */
   draw = ()  => {
      ...
      if (this.#animatePoint) {
        xDotPosition = 200 * Math.sin((this.#alpha * this.#pointLocation) + this.#radians);
